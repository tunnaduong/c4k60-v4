...
<html>
  ...
  <head>
      ...
      <title>Thư viện ảnh C4K60 - Example Video</title>
...
?>
    ...
    <a class="breadcrumb-item" href="index.php">Trang chủ</a><a class="breadcrumb-item" href="example_video_album.php">Example Video</a>
   <div id="tunna">
...
$sql = "SELECT * FROM videos WHERE album = 2";
...
$results = mysqli_query($db, $sql);
if (mysqli_num_rows($results)){
$sql = "SELECT * FROM videos WHERE type='youtube' AND album = 2";
$results = mysqli_query($db, $sql);
if (mysqli_num_rows($results)){
      while($row=mysqli_fetch_assoc($results)){
?>
...
?>
          <?php
  require_once $_SERVER['DOCUMENT_ROOT'] . '/login/serverconnect.php';
$sql = "SELECT * FROM videos WHERE type='html5' AND album = 2";
$results = mysqli_query($db, $sql);
if (mysqli_num_rows($results)){
      while($row=mysqli_fetch_assoc($results)){
?>
